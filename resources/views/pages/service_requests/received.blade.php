...
@section('default-content')
    <section class="px-4 pb-4">
            ...
            <thead class="bg-white sticky top-0">
                <tr>
                    <x-table-column title="from"
                        routeURL="{{ route('serviceRequests.received') }}?sortColumn=users.name&sortOrder={{ $sortOrder }}" />
                    <x-table-column title="request"
                        routeURL="{{ route('serviceRequests.received') }}?sortColumn=services.title&sortOrder={{ $sortOrder }}" />
                    <x-table-column title="date"
                        routeURL="{{ route('serviceRequests.received') }}?sortColumn=created_at&sortOrder={{ $sortOrder }}" />
                    <x-table-column title="status"
                        routeURL="{{ route('serviceRequests.received') }}?sortColumn=status&sortOrder={{ $sortOrder }}" />
                    <th scope="col" class="p-4 lg:p-5 ">
                    </th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($serviceRequestsReceived as $serviceRequest)
                    @php
                    ...
                    <tr class="hover:bg-gray-100 cursor-pointer"
                        ...
                        <td class="flex items-center p-4 mr-12 space-x-6 whitespace-nowrap lg:p-5 lg:mr-0">
                            <img class="w-8 h-8 rounded" src="{{ asset('storage/avatars/' . $requestSender->avatar) }}"
        ...
        @if (count($serviceRequestsReceived) < 1)
